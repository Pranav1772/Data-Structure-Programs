...
{                                       //FUNCTION FOR SORT ARRAY IN ASCENING ORDER USING INSERTION SORT METHOD
    for (int i = 0; i < len; i++)
    {
        int key = arr[i], j = i - 1;
        while (j >= 0 && key < arr[j])
...
void DescendingOrder(int arr[], int len) //FUNCTION FOR SORT ARRAY IN DESCENING ORDER USING INSERTION SORT METHOD
{
    for (int i = 0; i < len; i++)
    {
        int key = arr[i], j = i - 1;
